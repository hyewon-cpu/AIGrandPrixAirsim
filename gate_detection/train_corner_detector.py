...
"""Train a gate-corner detector from LabelMe annotations.
...
DEFAULT_SPLIT_DIR = Path(__file__).resolve().parent / "dataforuse" #resolve() : turns to absolute path 
...
def load_pairs_file(pairs_file: str | Path) -> list[tuple[Path, Path]]: #read the pairs file and make it in a list of tuples, wchich contains the paths.  
    ...
    for raw_line in pairs_path.read_text(encoding="utf-8").splitlines():
        ...
        image_candidates = [part for part in parts if part.lower().endswith((".png", ".jpg", ".jpeg", ".bmp", ".webp"))]
...
class WeightedHeatmapMSELoss(nn.Module):
    ...
    def __init__(self, positive_weight: float = 20.0):
        super().__init__()
...
class LabelMeCornerDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        image_bgr, record = self._load_record(json_path)
...
def load_image_for_inference(image_path: Path, expected_height: int, expected_width: int) -> tuple[np.ndarray, torch.Tensor]:
    image_bgr = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
    if image_bgr is None:
        raise FileNotFoundError(f"Failed to read image: {image_path}")
    image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
    ...
    image_tensor = torch.from_numpy(image_rgb.astype(np.float32) / 255.0).permute(2, 0, 1).contiguous()
    return image_bgr, image_tensor
...
def save_prediction_record(
    ...
    for corner in predicted_corners:
        ...
        cv2.circle(overlay, (int(round(x)), int(round(y))), 4, (0, 0, 255), -1)
        cv2.putText(
            overlay,
            f"{label}:{score:.2f}",
    ...
    *,
    ...
    with pairs_out_path.open("w", encoding="utf-8") as pairs_out:
        ...
        for image_path, json_path in pairs:
            image_bgr, image_tensor = load_image_for_inference(image_path, expected_height, expected_width)
...
def build_args():
    ...
    parser.add_argument("--output_dir", type=str, default=str(script_dir / "corner_checkpoints"))
    parser.add_argument("--inference_output_dir", type=str, default=str(script_dir / "corner_inference"))
    parser.add_argument("--checkpoint", type=str, default=str(script_dir / "corner_checkpoints" / "best.pt"))
...
def main():
    ...
    model = CornerUNet().to(device)
    split_dir = Path(args.split_dir).expanduser().resolve() #expanduser : expand to your home direcotory if path starts with ~. resolve() : resolve to absolute path.
    if args.mode == "train":
