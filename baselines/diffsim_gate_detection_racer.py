...
class CornerHeatmapEstimator:
    def __init__(
        ...
        input_size=DEFAULT_CORNER_INPUT_SIZE,
        device="cpu",
    ):
        if torch is None:
            raise ImportError(
    ...
    def _preprocess_rgb(self, rgb_image: np.ndarray) -> torch.Tensor:
        ...
        resized = cv2.resize(rgb_image, (self.input_size, self.input_size), interpolation=cv2.INTER_AREA)
        resized = resized.astype(np.float32) / 255.0
        tensor = torch.from_numpy(np.transpose(resized, (2, 0, 1))[None, ...])
...
class DiffsimGateDetectRacer(DiffSimRacer):
    def __init__(
        ...
        corner_input_size=DEFAULT_CORNER_INPUT_SIZE,
        ...
        **kwargs,
        ...
        self.corner_estimator = CornerHeatmapEstimator(
            ...
            input_size=corner_input_size,
            device=kwargs.get("device", "cpu"),
        )
        self.corner_input_size = int(corner_input_size)
    ...
    def _viz_corner_overlay(self, rgb_image: np.ndarray, gate_candidates: list[dict]) -> None:
        ...
        cv2.imshow("corner_overlay", overlay)
        cv2.waitKey(1)
...
def build_args():
    ...
    parser.add_argument(
        "--corner_input_size",
        type=int,
        default=DEFAULT_CORNER_INPUT_SIZE,
        help="Input size used for the corner detector.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Show corner heatmaps every N frames (reduces UI overhead).",
    )
...
def main():
    ...
    racer = DiffsimGateDetectRacer(
        ...
        corner_input_size=args.corner_input_size,
